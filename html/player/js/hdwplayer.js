...
function setResize(){
	...
	for (var i = 0; i < cusid_ele.length; ++i) {
		...
		if(w != '' && w < pl[item.id].width ){
			...
			item.style.width = pl[item.id].width+'px';
			item.style.height = pl[item.id].height+'px';
		}
	}
}
	...
	if(q.video != undefined){
		switch (q.type) {
			...
			case 'rtmp':
				...
				if (isMobile().Android()) {
					_rtsp = q.rtsp;
					opt += '<a href="'+_rtsp+'" style="display:block;"><img border="0" width="100%" height="100%" src="'+_thumb+'">';
					opt += '</a>';
				} else {
...
function chgcnt(){
	...
	case 'rtmp':
		...
		if (isMobile().Android()) {
			...
			opt += '<a href="'+_rtsp+'"><img border="0" width="100%" height="100%" src="'+_thumb+'">';
			opt += '</a>';
		} else {
			opt += '<video poster="' + _thumb + '" onclick="this.play();" width="100%" height="100%" controls>';
			opt += '<source src="' + _hls + '" />';
		...
		break;
	default:
		opt += '<video  poster="' + hdw_pltXml.thumb + '" onclick="this.play();" width="100%" height="100%" controls>';
...
function gc(){
	if(q.config != undefined){
		...
		hdw_cxmlObj=hdw_cxmlDoc[q.id].documentElement;
		getConfig();
	}
}
...
function getConfig(){
	if(q.config != undefined){
		q.playListXml=hdw_cxmlDoc[q.id].documentElement.getElementsByTagName("playListXml")[0].innerHTML?hdw_cxmlDoc[q.id].documentElement.getElementsByTagName("playListXml")[0].innerHTML:'';	
	}
}
...
function getvideo(id){
	...
	hdw_pltXml.rtsp = html5_plx[q.id][id].getElementsByTagName("hls")[0] != undefined ? html5_plx[q.id][id].getElementsByTagName("hls")[0].innerHTML : '';
	chgcnt();
}
