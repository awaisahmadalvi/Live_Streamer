...
GMainLoop *loop;
GstElement *pipeline, *aSource, *vSource, *aRateConv, *flvmux, *bin, *sinkLocal,
		*sinkLive, *tee, *filter, *qLive, *qAudio, *qCam, *qLocal, *parse,
		*lclProgres, *liveProgres, *muxProgres, *aacmux;
...
void stop();
char * getJsonValue(char * jKey, char[]);
...
int initialize(int argc, char *argv[]) {
	...
	vCap = gst_caps_from_string((gchar *) getJsonValue("v4l2caps", val));
	aCap = gst_caps_from_string((gchar *) getJsonValue("audiocaps", val));
	//filter 		= gst_element_factory_make ("capsfilter",	"filter");
	//valve     	= gst_element_factory_make ("valve", "live-valve");
	bin = gst_bin_new("liveBin");

	if (!pipeline || !aSource || !aRateConv || !vSource || !qAudio || !qCam
			|| !qLocal || !qLive || !flvmux || !tee || !sinkLocal || !sinkLive
			|| !vCap || !aCap || !lclProgres || !liveProgres
//		    || !filter || !valve
			|| !bin || !aacmux ) {
...
void setProp() {
	...
	sprintf(loc, "%s%s", (gchar *) getJsonValue("location", val), timeStr);
	...
	sprintf(serverIp, "rtmp://%s/%s/%s",
			(gchar *) getJsonValue("serverIp", val),
			(gchar *) getJsonValue("serverApp", val),
			(gchar *) getJsonValue("streamId", val));
...
int linkAll() {
	...
	gst_bin_add_many(GST_BIN(pipeline), vSource, aSource, aRateConv, qCam,
			qAudio, parse, flvmux, muxProgres, tee, qLocal, lclProgres,
			sinkLocal, qLive, bin, aacmux, NULL); // liveProgres, sinkLive,
