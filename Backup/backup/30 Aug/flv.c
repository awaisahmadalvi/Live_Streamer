...
int main (int argc, char *argv[]) {
	...
	GstElement	*pipeline, *source, *flvmux, *qCam, *qLocal, *muxProgres, 
			...
			*parse, *lclProgres, *liveProgres, *valve;
	GstBus *bus;
	guint bus_watch_id;
	GstCaps *cap;
	GstPad *pad;
	...
	/*
		gst-launch-1.0 -v v4l2src device=/dev/video2 ! video/x-h264, framerate=10/1, width=640, height=480 ! queue2 ! h264parse config-interval=96 ! 
		progressreport name=mux ! flvmux ! tee name=t ! queue2 ! progressreport name=local ! filesink location=mux.flv t. ! queue2 ! progressreport name=live !
		rtmpsink location=rtmp://103.226.217.136:1935/live/myStream
	...
	*/
	...
	/* we set the live progress report frequency to 2 seconds */
	/*g_object_set (qCam, "max-size-time", 100000000000, NULL);
	g_object_set (qCam, "max-size-buffers", 10000, NULL);
	g_object_set (qCam, "max-size-bytes", 1000000000000, NULL);
	...
	g_signal_connect(qCam, "overrun", G_CALLBACK (queue_overrun), NULL);


	/* we add all elements into the pipeline */
	gst_bin_add_many (GST_BIN (pipeline), source, qCam, parse, flvmux, muxProgres, tee, qLocal, lclProgres, sinkLocal, qLive, liveProgres, sinkLive, NULL);
