...
static void queue_overrun(GstElement *queue, gpointer data) {
	...
	g_print ("Status: %u, %u, %u, %u, %u, %u\n",mBuf, cBuf, mByt/1000/1000, cByt/1000/1000, mTim/100000000, cTim/100000000);
}
...
int main (int argc, char *argv[]) {
	...
	GstElement	*pipeline, *aSource, *vSource, *aConvert, *flvmux, *aEncoder, *bin,
			*sinkLocal, *sinkLive, *tee, *filter, *qLive, *qAudio, *qCam,
			*qLocal, *parse, *lclProgres, *liveProgres, *muxProgres;
	GstBus *bus;
	guint bus_watch_id;
	GstCaps *cap;
	GstPad *pad;
	...
	/*
		...
		gst-launch-1.0 pulsesrc device="alsa_input.usb-HD_Camera_Manufacturer_HD_USB_Camera_SN0008-03-Camera.analog-mono" ! audiorate ! voaacenc perfect-timestamp=1 hard-resync=1 bitrate=64000 ! progressreport name=audioprogress ! queue ! flvmux name=m ! progressreport name=muxprogress ! tee name=t ! queue ! progressreport name=local ! filesink location=mux.flv t. ! queue leaky=2 ! progressreport name=liveprogress ! rtmpsink location=rtmp://103.226.217.136:1935/live/myStream async=1 v4l2src device=/dev/video2 ! video/x-h264, framerate=5/1, width=640, height=480 ! h264parse ! progressreport name=videoprogress ! queue ! m.

		gst-launch-1.0 pulsesrc device="alsa_input.usb-HD_Camera_Manufacturer_HD_USB_Camera_SN0008-03-Camera.analog-mono" ! audiorate ! speexenc ! progressreport name=audioprogress ! queue ! flvmux name=m ! progressreport name=muxprogress ! queue leaky=2 ! progressreport name=liveprogress ! rtmpsink location=rtmp://103.226.217.136:1935/live/myStream async=1 
	...
	*/
	...
	aSource   	= gst_element_factory_make ("pulsesrc",		"pulse-aSource");
	aConvert   	= gst_element_factory_make ("audiorate",	"audio-converter");
	aEncoder   	= gst_element_factory_make ("speexenc",		"aac-encoder");
	...
	cap 	 	= gst_caps_from_string ("video/x-h264,framerate=5/1,width=640,height=480");
	...
	g_object_set (G_OBJECT (vSource), "device", "/dev/video2", NULL);

	/* we set the input Pulse Audio device to the aSource element */
	g_object_set (G_OBJECT (aSource), "device", "alsa_input.usb-HD_Camera_Manufacturer_HD_USB_Camera_SN0008-03-Camera.analog-mono", NULL);
	...
	g_object_set (G_OBJECT (sinkLocal), "location", "mux.flv", NULL);
	...
	g_object_set (qLive, "max-size-buffers", 0, NULL);		//50, 200
	...
	gst_bin_add_many (GST_BIN (pipeline), vSource, aSource, aConvert, aEncoder, qCam, qAudio, parse, flvmux, muxProgres, tee, qLocal, lclProgres, sinkLocal, qLive, bin, NULL); // liveProgres, sinkLive,
	

	/* we link the elements together */
	gst_element_link_filtered((GstElement*) vSource, qCam, cap);
	gst_element_link_many(aSource, aConvert, aEncoder, qAudio, flvmux, NULL);
