...
int main (int argc, char *argv[]) {
	...
	GstElement *pipeline, *source, *avimux, *flvmux,*queueLocal, *queueLive, *sinkLocal, *sinkLive, *tee, *filter;
	GstBus *bus;
	guint bus_watch_id;
	GstCaps *cap;
	gint res;
	...
	avimux  	= gst_element_factory_make ("avimux",	"avi-mux");
	...
	if ( !pipeline || !source || !avimux || !flvmux || !queueLocal || !queueLive || !tee || !sinkLocal || !sinkLive  || !cap) {
	...
	gst_bin_add_many (GST_BIN (pipeline), source, avimux, flvmux ,queueLocal, queueLive, tee, sinkLocal, sinkLive, NULL);
	...
	gst_element_link_filtered((GstElement*) source, tee, cap);
	gst_element_link_many (queueLocal, flvmux,  sinkLocal, NULL);
	gst_element_link_many (queueLive, avimux, sinkLive, NULL);
