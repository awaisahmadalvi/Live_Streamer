...
int main (int argc, char *argv[]) {
	...
	/* 
		gst-launch-1.0 v4l2src device=/dev/video2 ! video/x-h264, framerate=30/1, width=640, height=480 ! flvmux ! tee name=t t. ! queue ! progressreport ! 			filesink location=test.flv t. ! queue ! rtmpsink location=rtmp://103.226.217.136:1935/live/myStream
	*/
	...
	cap 	 	= gst_caps_from_string ("video/x-h264, framerate=30/1, width=640, height=480");
	bin		= gst_bin_new ("liveBin");
	
	if ( !pipeline || !source || !flvmux || !queueLocal || !queueLive || !tee || !sinkLocal || !sinkLive  || !cap || !progReport || !bin || !valve ) {
	...
	g_object_set (G_OBJECT (sinkLive), "location", "rtmp://103.226.217.136/live/myStream", NULL);//"location", "temp2.flv", NULL);
	
	/* we set the output to PAUSE async to filesink */
	g_object_set (G_OBJECT (sinkLive), "async", TRUE, NULL);//"location", "temp2.flv", NULL);
	...
	gst_bin_add_many (GST_BIN (pipeline), source, flvmux, queueLocal, tee, queueLive, sinkLocal, bin, NULL); //sinkLive, progReport, 
