...
GMainLoop 	*loop;
GstElement	*pipeline, *aSource, *vSource, *aRateConv, *flvmux, *bin,
...
int initialize (int argc, char *argv[]) {
	...
	/*
		...
		gst-launch-1.0 pulsesrc device="alsa_input.usb-HD_Camera_Manufacturer_HD_USB_Camera_SN0008-03-Camera.analog-mono" ! audiorate ! faac perfect-timestamp=1 hard-resync=1 bitrate=64000 ! progressreport name=audioprogress ! queue ! flvmux name=m ! progressreport name=muxprogress ! tee name=t ! queue ! progressreport name=local ! filesink location=mux.flv t. ! queue leaky=2 ! progressreport name=liveprogress ! rtmpsink location=rtmp://103.226.217.136:1935/live/myStream async=1 v4l2src device=/dev/video2 ! video/x-h264, framerate=5/1, width=640, height=480 ! h264parse ! progressreport name=videoprogress ! queue ! m.


		gst-launch-1.0 pulsesrc device="alsa_input.usb-HD_Camera_Manufacturer_HD_USB_Camera_SN0008-03-Camera.analog-mono" ! audiorate ! progressreport name=audioprogress ! queue ! flvmux ! progressreport name=muxprogress ! queue leaky=2 ! progressreport name=liveprogress ! rtmpsink location=rtmp://103.226.217.136:1935/live/myStream async=1
...
//	filter 		= gst_element_factory_make ("capsfilter",	"filter");
	vCap 	 	= gst_caps_from_string ("video/x-h264,framerate=10/1,width=640,height=480");
	aCap 	 	= gst_caps_from_string ("audio/x-raw,rate=44100,channels=1");
...
//	g_object_set(filter,"caps",vCap,NULL);	
	...
	g_object_set (G_OBJECT (sinkLocal), "location", timeStr, NULL);
...
int linkAll () {
	...
	gst_bin_add_many (GST_BIN (pipeline), vSource, aSource, aRateConv, qCam, qAudio, parse, flvmux, muxProgres, tee, qLocal, lclProgres, sinkLocal, qLive, bin, NULL); // liveProgres, sinkLive,
	

	/* we link the elements together */
	gst_element_link_filtered((GstElement*) vSource, qCam, vCap);
	gst_element_link_filtered((GstElement*) aSource, aRateConv, aCap);
	gst_element_link_many(aRateConv, qAudio, flvmux, NULL);
...
int main (int argc, char *argv[]) {
	...
	guint ret = initialize (argc, argv);
	if (ret < 0)
		return -1;
	setProp ();
	ret = setTee ();
