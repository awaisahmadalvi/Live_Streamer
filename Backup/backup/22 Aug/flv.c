...
int main (int argc, char *argv[]) {
	...
	GstElement *pipeline, *source, *avimux, *flvmux, *queueLocal, 
			*queueLive, *sinkLocal, *sinkLive, *tee, *filter,
			*queueNew, *progReport, *bin, *valve, *parse;
	GstBus *bus;
	guint bus_watch_id;
	GstCaps *cap;
	GstPad *pad;
	...
	/* 
		gst-launch-1.0 v4l2src device=/dev/video2 ! video/x-h264,width=640,height=480,framerate=10/1 ! matroskamux streamable=true ! tee name=t t. ! 			queue ! filesink location=temp.flv t. ! queue ! bin name=b b. ! valve drop=false ! progressreport update-freq=2 ! 
		rtmpsink location=rtmp://103.226.217.136:1935/live/myStream async=true
	*/

	/*
		gst-launch-1.0 v4l2src device=/dev/video1 ! video/x-h264,width=1280,height=720,framerate=30/1,profile=high ! h264parse ! flvmux ! progressreport ! 
		rtmpsink location=rtmp://103.226.217.136/live/myStream
	*/
	...
	cap 	 	= gst_caps_from_string ("video/x-h264,width=640,height=480,framerate=10/1,profile=baseline");
	bin		= gst_bin_new ("liveBin");
	
	if ( !pipeline || !source || !flvmux || !queueLocal || !queueLive || !tee || !sinkLocal || !sinkLive  || !cap || !progReport || !bin || !valve || !filter  || !parse ) {
	...
	g_object_set (G_OBJECT (flvmux),"streamable", 1, NULL);	
	...
	g_object_set (G_OBJECT (sinkLive), "sync", TRUE, NULL);
	...
	gst_bin_add_many (GST_BIN (pipeline), source, parse, flvmux, queueLocal, tee, queueLive, sinkLocal, bin, NULL); //sinkLive, progReport, 
	
	gst_element_link_filtered((GstElement*) source, parse, cap);

	gst_element_link_many(parse, flvmux, tee, NULL);
